...
// ─── SLIDE 2 — Sommaire ─────────────────────────────────────────────────────
{
  ...
  const parts = [
    {
      ...
      items: ["4 métriques ISO 25010", "Cycle de vie DevSecOps", "Pipeline CI/CD détaillé", "Compétences & formation"],
      color: "2E1A1A",
    },
    {
      num: "02",
      title: "Développement & Déploiement",
      items: ["Backlog & User Stories", "Architecture BFF", "Expérimentation technologies", "Résultats & chiffres clés"],
      color: "1A1A2E",
    },
    {
      num: "03",
      title: "Sécurité & Remédiation",
      items: ["Analyse OWASP Top 10", "Plan de remédiation 3 sprints", "Mesures préventives v2", "Bilan de la mission"],
      color: "1A2E1A",
    },
  ];
  ...
  parts.forEach(({ num, title, items, color }, i) => {
    ...
    items.forEach((item, j) => {
      s.addText("▸  " + item, { x: x + 0.2, y: 4.5 + j * 0.57, w: 3.6, h: 0.5, fontSize: 11, color: C.grey, fontFace: FONT_BODY });
    });
  });
  s.addNotes("La présentation se découpe en trois parties. D'abord le processus qualité — comment on garantit que ce qu'on livre est bien fait. Ensuite la réalisation concrète du prototype. Et enfin l'analyse de sécurité avec un plan d'action.");
}
...
// ─── SLIDE 4 — 4 Indicateurs ISO 25010 ──────────────────────────────────────
{
  ...
  metrics.forEach(({ label, val, iso, tool, color }, i) => {
    ...
    s.addText(tool, { x: x + 0.1, y: 5.7, w: 2.8, h: 0.4, fontSize: 10, color: C.grey, fontFace: FONT_BODY, align: "center", italic: true });
  });
...
// ─── SLIDE 5 — DevSecOps Cycle de vie ────────────────────────────────────────
{
  ...
  steps.forEach(({ name, tools, color }, i) => {
    ...
    s.addText(tools, { x: x + 0.1, y: 3.85, w: 2.1, h: 1.5, fontSize: 11, color: C.grey, fontFace: FONT_BODY, align: "center" });
...
// ─── SLIDE 7 — Compétences & Formation ──────────────────────────────────────
{
  ...
  profiles.forEach(({ role, skills, nb, color }, i) => {
    ...
    s.addText(skills, { x: x + 0.1, y: 4.25, w: 2.8, h: 1.2, fontSize: 11, color: C.grey, fontFace: FONT_BODY, align: "center" });
  });
...
// ─── SLIDE 8 — Séparateur Partie 2 ──────────────────────────────────────────
{
  ...
  s.addText("Backlog · Architecture · Expérimentation · Résultats", {
    x: 0, y: 4.4, w: "100%", h: 0.5,
    fontSize: 14, color: C.grey, fontFace: FONT_BODY, align: "center", italic: true,
  });
...
// ─── SLIDE 9 — Backlog / User Stories ───────────────────────────────────────
{
  ...
  us.forEach(({ id, title, detail, color, tag }, i) => {
    ...
    s.addText(detail, { x: x + 0.15, y: y + 1.05, w: 6.0, h: 0.7, fontSize: 10, color: C.grey, fontFace: FONT_BODY });
  });
...
// ─── SLIDE 11 — Expérimentation ─────────────────────────────────────────────
{
  ...
  addRedBar(s);
  addBottomBar(s);
  sectionTitle(s, "BAC A SABLE — EXPERIMENTATION");
  mainTitle(s, "5 technologies testées\navant développement");
...
// ─── SLIDE 12 — Résultats des tests ─────────────────────────────────────────
{
  ...
  stats.forEach(({ val, sub, detail, color }, i) => {
    ...
    s.addText(detail, { x: x + 0.1, y: 5.2, w: 2.8, h: 0.5, fontSize: 10, color: C.grey, fontFace: FONT_BODY, align: "center" });
  });
...
// ─── SLIDE 15 — Plan de remédiation ─────────────────────────────────────────
{
  ...
  sprints.forEach(({ sprint, actions, impact, color, impactColor }, i) => {
    ...
    s.addText(actions, { x: x + 0.2, y: 3.95, w: 3.6, h: 2.4, fontSize: 11, color: C.grey, fontFace: FONT_BODY, paraSpaceAfter: 4 });
...
// ─── SLIDE 16 — Mesures préventives v2 ──────────────────────────────────────
{
  ...
  measures.forEach(({ num, title, desc, icon, color }, i) => {
    ...
    s.addText(desc, { x: x + 0.2, y: 4.6, w: 3.6, h: 2.0, fontSize: 11, color: C.grey, fontFace: FONT_BODY });
  });
